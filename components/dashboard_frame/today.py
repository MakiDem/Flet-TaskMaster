...
from database.crud import get_today_tasks
    ...
    return ft.Row(
        [
            ...
            content=ft.Column([
                ...
                ft.Column(
                    [create_taskcard(task_text=task['title'], is_completed=(task['status'] == 'completed')) for task in get_today_tasks()],
                    spacing=10,
                ),
            ]),
                ...
                padding=25,
                border_radius=15,
                expand=True  # makes container take full width
            ),
            # Progress Card
